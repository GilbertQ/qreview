...
const QuizComponent = () => {
  ...
  const shouldShowRepaso = percentageCorrect < 80 && wrongAnswers.length > WRONG_ANSWER_LIMIT;

if (shouldShowRepaso) {
  setQuizData(null); // Reset quiz data when limit is reached
}
  ...
  return (
    ...
    {/* Conditionally render pauseIt */}
    {shouldShowRepaso && !isReviewingWrongAnswers && quizData === null && renderPauseIt()}
        </>
      )}
    </Box>
  );
